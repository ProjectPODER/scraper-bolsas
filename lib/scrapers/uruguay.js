...
const bolsa = 'Bolsa de Valores de Montevideo';

async function getList() {
    const listURL = 'https://www.bvm.com.uy/operadores/emisores-de-acciones';

    try {
        var browser = await puppeteer.launch({ headless: true });
        var page = await browser.newPage();
        await page.goto(listURL, { waitUntil: 'networkidle0' });
        await page.waitForSelector('div.contCorredores div.bloqueCorredor');

        let list = await page.$$eval('div.contCorredores div.bloqueCorredor', divs => divs.map(div => {
            let nombre = div.querySelector('h4').textContent.trim();
            let direccion = '';
            let telefono = '';
            let website = '';
            let email = '';

            let spans = Array.from(div.querySelectorAll('span.datoCorredor'));
            spans.map( (span, i) => {
                switch(i) {
                    case 0:
                        direccion = span.textContent.trim();
                        break;
                    case 1:
                        telefono = span.textContent.trim();
                        break;
                }
            } );

            let links = Array.from(div.querySelectorAll('a.datoCorredor'));
            links.map( (link, i) => {
                switch(i) {
                    case 0:
            ...
            let button = div.querySelector('div.links_operadores > a:nth-child(1)');

            return { nombre, direccion, telefono, website, email, button }
        }));
        ...
        for(let i=0; i<list.length; i++) {
            let item = list[i];
            let company = buildCompany(item);
            await page.$eval('div.contCorredores div.bloqueCorredor:nth-child(' + (i+1) + ') div.links_operadores > a:nth-child(1)', button => button.click() );
            await page.waitForSelector('#cboxLoadedContent .texto_operador');
            let details = await page.$$eval('#cboxLoadedContent .texto_operador p', textos => textos.map(texto => {
...
function buildCompany(row) {
    let company = {
        id: laundry.simpleName(laundry.launder(row.nombre)),
        name: row.nombre,
        classification: 'company',
        area: [{
            id: 'uy',
